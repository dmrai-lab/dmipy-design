...
def test_min_te_reaches_target_b_and_is_tight():
    ...
    d, te = min_te_for_b(b_target, b_delta=1.0, limits=LIM, timing=TIMING,
    ...
    lo = te - 4e-3
    if lo > TIMING.min_TE():
        d_lo = design_waveform_now(1.0, limits=LIM, TE=lo, timing=TIMING, n_t=64, n_restarts=3, seed=0)
...
def test_shorter_target_gives_shorter_te():
    ...
    _, te_small = min_te_for_b(2e8, b_delta=1.0, limits=LIM, timing=TIMING, n_t=64, n_restarts=3, tol_te=2e-3)
    _, te_big = min_te_for_b(6e8, b_delta=1.0, limits=LIM, timing=TIMING, n_t=64, n_restarts=3, tol_te=2e-3)
    assert te_small < te_big


def test_unreachable_target_raises():
    with pytest.raises(ValueError):
        min_te_for_b(1e12, b_delta=1.0, limits=LIM, timing=TIMING, n_t=48, n_restarts=2,
