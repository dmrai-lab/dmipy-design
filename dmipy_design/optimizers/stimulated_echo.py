...
timing/structure — but, crucially, it **reuses the entire encoding optimizer core
unchanged** (``design_waveform_now``): the diffusion encoding is still described by
the same effective wavevector ``q(t) = γ·cumsum(s·g)·dt`` and b-tensor
...
* **No 180 — the sign flip is the stimulated-echo conjugation.** The store/recall
  ...
  *gradient-off middle* (store/TM/recall) — exactly the role ``echo_idx`` plays in
  the spin-echo core, just placed at the storage rather than at TE/2.
* **Mixing time TM, gradient OFF, magnetization on z.** During TM the spins are
  ...
  **T1-limited (not T2-limited)** effective diffusion time — PGSTE's whole reason
  to exist.  A static field accrues NO phase during TM (only the transverse periods
  τ₁/τ₃ matter).
* **Static refocus needs MATCHED encoding periods τ₁ = τ₃** — the PGSTE analog of
  "180 at TE/2".  The stimulated echo refocuses a static field only when the two
...
* **The storage factor and T1 weighting** are signal/relaxation effects (applied at
  playback time), not encoding-design constraints.

Scope
-----
This module designs the *effective* PGSTE encoding (``q``, b-tensor, shape, hardware,
optional M1/M2/Maxwell/spectral/PNS/heat all apply identically through the NOW oracle).
It returns a :class:`~dmipy_design.optimizers.now.NowDesign`.  Under the instant-pulse
public scope this is a design artefact; running it as a real 3×90 stimulated echo (RF
playback) is out of scope here.
"""
...
class StimulatedEchoTiming:
    ...
    def masks(self, TE=None, n_t=256):
        ...
        gradient-off middle.  Duck-compatible with ``design_waveform_now(timing=…)``.
        """
...
def design_stimulated_echo(b_delta, *, TM, TE, t_excite=2e-3,
                           ...
                           **design_kwargs):
    """Design a PGSTE diffusion-encoding waveform.

    Builds a :class:`StimulatedEchoTiming` for the given mixing time ``TM`` and echo
    time ``TE`` and hands it to the NOW design oracle (``design_waveform_now``) via its
    duck-typed ``timing`` interface — so b-tensor shape, refocusing, hardware limits,
    and the optional M1/M2/Maxwell/spectral/PNS/heat constraints all apply unchanged.
    Returns a :class:`~dmipy_design.optimizers.now.NowDesign` whose gradient is the
    *effective* PGSTE encoding (the long-TM hold gives the long, T1-limited diffusion
    time).  ``design_kwargs`` are forwarded to ``design_waveform_now``.
    """
    ...
    return design_waveform_now(b_delta, timing=timing, TE=TE, **design_kwargs)
